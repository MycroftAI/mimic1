 ...
 */
static int regtry(cst_regstate *state,
                  const char *string, char *prog) /* 0 failure, 1 success */
{
    int i;
    const char **sp;
    const char **ep;
 ...
 * by recursion.
 */
static int regmatch(cst_regstate *state,
                    char *scan) /* 0 failure, 1 success */
{
    char *next;                 /* Next node. */
    ...
    while (scan != NULL)
    {
        ...
        case ANYOF:
            if (*state->input == '\0'
                || strchr(OPERAND(scan), *state->input) == NULL)
                return (0);
            state->input++;
            break;
        case ANYBUT:
